...
def add(message):
    if not str(message.chat.id) in joinedUsers1:
        ...
        joinedFile1.write(str(message.chat.id) + '\n')
        joinedUsers1.add(message.chat.id)

@bot.message_handler(commands=['it1'])
def add(message):
    if not str(message.chat.id) in joinedUsers2:
        joinedFile2 = open('data/it1.txt', 'a')
        joinedFile2.write(str(message.chat.id) + '\n')
        joinedUsers2.add(message.chat.id)

@bot.message_handler(commands=['it2'])
def add(message):
    if not str(message.chat.id) in joinedUsers3:
        joinedFile3 = open('data/it2.txt', 'a')
        joinedFile3.write(str(message.chat.id) + '\n')
        joinedUsers3.add(message.chat.id)

@bot.message_handler(commands=['it3'])
def add(message):
    if not str(message.chat.id) in joinedUsers4:
        joinedFile4 = open('data/it3.txt', 'a')
        joinedFile4.write(str(message.chat.id) + '\n')
        joinedUsers4.add(message.chat.id)
#расписание меню
...
def callback_inline(call):
    try:
        if call.message:
            ...
            with open('data/it1.txt', 'r', encoding='UTF-8') as far: ## Открываем файл
                test2 = list(far) ## Помещаем в список.
                test3 = list(map(int, test2))
            with open('data/it2.txt', 'r', encoding='UTF-8') as fara: ## Открываем файл
                test4 = list(fara) ## Помещаем в список.
                test5 = list(map(int, test4))
            with open('data/it3.txt', 'r', encoding='UTF-8') as farad: ## Открываем файл
